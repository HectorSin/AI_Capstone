...
from app.schemas import (
    AIJob, AIJobCreate, AIJobUpdate, JobStatus, JobType, PodcastCreate, PodcastCreateResponse
)
...
@router.post("/test", response_model=PodcastCreateResponse, summary="AI 팟캐스트 생성 (테스트용)")
async def create_ai_podcast_test(
    ...
    try:
        # 테스트용 사용자 ID 설정
        podcast_data.user_id = "test-user-id"
        ...
        logger.error(f"AI 팟캐스트 생성 실패: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/", response_model=PodcastCreateResponse, summary="AI 팟캐스트 생성")
async def create_ai_podcast(
    ...
    try:
        # 사용자 ID 설정
        podcast_data.user_id = current_user.user_id
